...
import { evm, PaymentRequirements, ChainIdToNetwork, Signer, MultiNetworkSigner } from "@paylatch/x402/types";
import { withPaymentInterceptor } from "./index";

// Mock the createPaymentHeader function
vi.mock("@paylatch/x402/client", () => ({
...
describe("withPaymentInterceptor()", () => {
  ...
  beforeEach(async () => {
    ...
    const { selectPaymentRequirements } = await import("@paylatch/x402/client");
  ...
  it("should reject if payment header creation fails", async () => {
    ...
    const { createPaymentHeader } = await import("@paylatch/x402/client");
...
describe("withPaymentInterceptor() - SVM and MultiNetwork", () => {
  ...
  it("passes [solana, solana-devnet] for SVM-only signers", async () => {
    vi.doMock("@paylatch/x402/types", async () => {
      const actual = await vi.importActual("@paylatch/x402/types");
      return {
        ...actual,
    ...
    vi.doMock("@paylatch/x402/client", () => ({
